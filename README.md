...
定义好根目录（省），下面就按照目录里的层级一级一级下去吧。市、区、街道、社区、路、号、楼、单元、间按照路径结构继续往下编，最多支持到7级目录。为什么是7级？为了区分目录和文件，需要把文件级别直接设定到“单元”，甚至扩展名最好也拆开，命名到“间”，比如：d:/output/图像数据输出/图像1.jpg，可以转化为：磁颐国输目省图像数据输出市图像1单元jpg间。然后读写文件就可以这样写：

```
访问磁颐国工根省测试数据市点单元yml间，加载数据为“多边形点集”。
将图片《图像1》另存到磁颐国输目省图像数据输出市图像1单元jpg间”。
```
...
可以改变图片大小：
```
将图片《lena.png》大小改为横：200，竖：100。
```
转换图片形式（二值、灰度、颜色索引、RGB、RGBA）
```
将图片《lena.png》改为“二值图”模式。
```
然后保存
```
将图片《lena.png》另存为：“d:/lena0101.jpg”。
```
也可以新建一张图片
```
