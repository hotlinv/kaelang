...
###### 获取当前城市温度
举个栗子：比如我们想自动获取当前城市现在的温度，要怎么做呢？

天气里包含当前温度，可以通过访问[这个](http://www.nmc.cn/rest/weather)接口来获取（这个接口的形式是： http://www.nmc.cn/rest/weather?stationid=XXXX ）。但是，这个接口必须需要先获取当前城市编码。当前城市编码可以从[这里](http://www.nmc.cn/rest/province)获取（这个接口的形式是： http://www.nmc.cn/rest/province 获取所有的省，再用 http://www.nmc.cn/rest/province/省编码 来获取该省的所有市编码），而这个接口没有参数来传入城市的名称，所以首先需要知道当前的城市是什么，所以我们从[这里](http://pv.sohu.com/cityjson?ie=utf-8)获取查询当前的城市名称。

所以我们需要分成3步来做：

####### 获取当前城市名称

首先我们需要修改urlmap.yml，注册网络请求的地址，以便让我们的url能使用中文方式访问到。


```
星辰国 :
 位处省: http://pv.sohu.com/cityjson?ie=utf-8
```

这样我们就可以使用这种方式来下载数据了：

```
访问位于星辰国位处省查询室文本间的数据“当前所处城市文本”。
```

`星辰国位处省` 就是url地址，但是下面两个大家很陌生，我需要解释一下：

- “查询室”是表达是个GET请求，同理，POST叫“新建室”， PUT叫“更新室”，DELETE叫“删除室”。

- “文本间”表示这个返回是个文本(plain/text)，“json间”表示这个返回是个json对象（会自动解析转换），“xml间”表示返回xml对象（也会自动解析转换）
...
```
var returnCitySN = {"cip": "220.160.87.213", "cid": "350100", "cname": "福建省福州市"};
```

这个其实是一个javascript……。所以我不会直接设置“json间”。好在它就是一行，且大部分内容是一个json。我们需要把它转成json。


```
解析《当前所处城市文本》中的对象“当前城市对象”。
```

它会把json串截出来，然后形成对象。我们就可以使用`《当前城市对象》的cname属性`来获取这个城市值了。

等等——`《当前城市对象》的cname属性`这种中英混写的写法相当恶心。我们需要转成纯中文的写法，所以需要将中英文属性名做个对应。我们在根目录下建一个目录叫“数据描述”，再建一个文件叫“当处城对象描述.yml”（名字可以随便起啦）。然后里面建立属性对应关系。
...
为了能把这个结构描述和对象挂钩，需要运行一行代码：


```
访问磁颐国工根省数据描述市当处城对象描述室yml间，加载数据为“城市对象描述”。以《城市对象描述》来描述《当前城市对象》。
```

好了。现在可以通过`《当前城市对象》的城市名称`来获取`《当前城市对象》的cname属性`了。


####### 获取当前城市所在省和市

获取省的数据使用 http://www.nmc.cn/rest/province ，如果直接调用（不传参数），会返回所有的省。
...
找到所在省后通过code属性，接着请求 http://www.nmc.cn/rest/province/{省code属性} ，就会返回市


```
[{"code":"58847","province":"福建省","city":"福州","url":"/publish/forecast/AFJ/fuzhou.html"},
...
,{"code":"58749","province":"福建省","city":"柘荣","url":"/publish/forecast/AFJ/zherong.html"}]
```

然后就能通过相应市的code属性拿到下一步需要的城市编码了。


代码其实也不难。首先在urlmap.yml注册地址


```
星辰国 :
 位处省: http://pv.sohu.com/cityjson?ie=utf-8
 城码省: http://www.nmc.cn/rest/province
```

然后建立一个请求所有省时返回的数据字段对应表（数据描述/省编码描述.yml）：
...
```
访问位于星辰国城码省查询室json间的数据“当前所处省编码表”。访问磁颐国工根省数据描述市省编码描述室yml间，加载数据为“省编码描述”。以《省编码描述》来描述《当前所处省编码表》。
查找《当前所处省编码表》中“名称”和《当前城市对象》的“城市名称”前半段相同的首条记录，并将其重命名为当前省。将《当前省》的编码命名为当前省之编码。
```

前面一行不用解释了，解释下下面一行。`查找《当前所处省编码表》中“名称”和《当前城市对象》的“城市名称”前半段相同的首条记录`，就是遍历刚才返回后的省信息列表，把每个省的“名称”属性和《当前城市对象》的“城市名称”属性两个字符串对比，找出具有相同开头的记录，返回。如果不加“首条”，就返回所有记录，加了“首条”只返回第一条。然后重命名为`当前省`。注意这里是记录被叫做`当前省`，要想得到当前省的编码，要使用`《当前省》的编码`。我们可以把`《当前省》的编码`重新命名为`当前省之编码`。以便后面来使用。

接下来要通过省拿市信息了，url位置是不变的，只是需要在地址后面加子路径，这个跟文件路径类似，市县乡……地理编码一级一级往下编就行。
...
`星辰国城码省当前省之编码市查询室json间`中的`当前省之编码市`用的就是我们刚才重命名的变量。重命名主要是为了拼接方便（也为了不引入符号且不引起地址混乱）。返回数据通过描述文件来对应，建立一个请求市信息时返回的数据字段对应表（数据描述/市编码描述.yml）：
...
然后对记录进行查询，和重命名。`有包含关系的首条记录`是因为我懒得做字符串拼接。


####### 获取当前城市天气

获取天气信息的方式和前面的稍微有点不一样。因为这个url带参数（ http://www.nmc.cn/rest/weather?stationid=城市编码 ）。所以我们先要准备一下参数。参数的结构也是通过描述文件来说明的。

建立一个请求市信息时返回的数据字段对应表（数据描述/天气请求参数.yml）：

```
编码: stationid
```

然后依据这个结构构建出一个对象（map），并设置相应的值。
...
```
{"msg":"success","code":0,"data":{"real":{"station":{"code":"58847","province":"福建省","city":"福州","url":"/publish/forecast/AFJ/fuzhou.html"},"publish_time":"2021-12-29 16:35","weather":{"temperature":17.5,"temperatureDiff":6.0,"airpressure":9999.0,"humidity":57.0,"rain":0.0,"rcomfort":56,"icomfort":-1,"info":"多云","img":"1","feelst":14.6},"wind":{"direct":"西北风","power":"微风","speed":""},
... 
 ,"radar":{"title":"福州","image":"/product/2021/12/29/RDCP/SEVP_AOC_RDCP_SLDAS_EBREF_AZ9591_L88_PI_20211229084800000.PNG?v=1640767721623","url":"/publish/radar/fu-jian/fu-zhou.htm"}}}
```

所以对照表会稍微复杂一点。建立一个请求天气信息时返回的数据字段对应表（数据描述/天气对象描述.yml）：
...
可以看到结构是一级一级描述进去的。但是也还好，只要建立需要对应的字段就可以了。


```
访问磁颐国工根省数据描述市天气对象描述室yml间，加载数据为“天气对象描述”。以《天气对象描述》来描述《当前市天气》。
打印：《当前城市对象》的城市名称、“当前温度是”、《当前市天气》的气温。
```
