...
# 【映射】
ka_pmap=lambda:{
	u"(?:访问|打开)在(.+)的图像文件“(.+)”":"ka_pil_open(ka_path('{0}'), '{1}')",
    u"(?:把|改变)图(?:像|片)《(.+)》大小(?:改|转)?为横：(\d+)(?:，)?\s*竖：(\d+)":"ka_pil_resize('{0}', {1}, {2})",
    u"(?:把|改变)图(?:像|片)《(.+)》(?:改|转)?为(\w+)模式":"ka_pil_convert('{0}', '{1}')",
    u"(?:把|改变)图(?:像|片)《(.+)》(?:的)?模式(?:改|转)?为(\w+)":"ka_pil_convert('{0}', '{1}')",
    u"(?:显示|展示)图(?:像|片)《(.+)》":"ka_pil_show('{0}')",
    u"把图(?:像|片)《(.+)》输出：(.+)":"ka_pil_save('{0}', ka_path('{1}'))",
